...
export async function GET(
  ...
  try {
    ...
    const includeReattempts = canManageQuizGlobally(session.user.role);
    ...
    const results = await prisma.$queryRaw<AnalyticsResultRow[]>`
      ...
      WHERE r."quizId" = ${quizId}
        AND (${includeReattempts} OR COALESCE(r."attemptType", 'NORMAL') = 'NORMAL')
    ...
    const formatted = results.map((r) => ({
      ...
      attemptType: r.attemptType ?? "NORMAL",
      score: r.score,
